...
- **协议取证工具（调试专属）**：`设置`(仅 debug) 提供两个入口。(1) `WebSocket 协议实验` → `运行实验`：`DebugWsExperimentScreen` 内藏 `1×1` 不可见 `WebView` 加载 `assets/ws_experiment.html`，其 JS 逐字节复刻 `ProtoCodec`/`ProtoCommands` 线协议，连机器实测 `d_prof_dict` + `g_prof(id)` 是否按 id 返回（详见附录 C / §0s）。(2) `WebUI 探测` → `运行探测`：`DebugWebUiProbeScreen` 内藏 `fillMaxSize().alpha(0.001f)` 不可见 `WebView` **直接加载机器提供的真实 WebUI SPA**，`shouldInterceptRequest` 记录所有 `/api/` 请求，`onPageFinished` 注入 `assets/ws_webui_probe.js` 的 `runProbe`：枚举每个按钮（只取 label 不 click）、正则提取 SPA JS 的全部 `/api/...` 端点、对未覆盖且非设置的端点模拟一次只读 GET 取回响应。**约束：只发 GET、不 click 按钮、不 POST/PUT，设置类端点仅列不模拟——绝不改动设置**。用于坐实「WebUI 有哪些按钮 / 哪些数据我们 App 没取」（详见 `GAGMATE_REFERENCE.md` 附录 C.1 / `CODE_REVIEW.md` §0t）。
