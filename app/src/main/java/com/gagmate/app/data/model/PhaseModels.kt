...
/**
 * Fill in durations for FLOW phases that declare a target but stop on the global
 * volume/weight limit (no per-phase time). Real flow-rate math, not a placeholder:
 *  - target volume `V` = [globalVolumeMl] (grams ≈ ml for water/espresso).
 *  - phases with an explicit time already know their poured volume → subtract
 *    those from `V` to get the *remaining* volume the volume-driven flow phases
 *    must pour.
 ...
 *
 * Receiver is [BrewPhase] list (used by both the shot-embedded path and the live
 * WebSocket `d_prof` path). Returns the same list unchanged when there is nothing
 * to estimate (no global volume, or every phase already has a duration).
 */
fun List<BrewPhase>.estimateVolumeDrivenTimes(globalVolumeMl: Float?): List<BrewPhase> {
    val gv = globalVolumeMl?.takeIf { it > 0f } ?: return this
    var knownVol = 0f
    ...
    forEachIndexed { i, p ->
        ...
        val end = p.target
        when {
            t > 1f && isFlow && end > 0f -> knownVol += end * t
            isFlow && end > 0f && t <= 1f -> { estIdx += i; sumFlow += end }
        }
    }
    if (estIdx.isEmpty() || sumFlow <= 0f) return this

    val remaining = maxOf(0f, gv - knownVol)
    val perPhaseSec = (remaining / sumFlow).coerceIn(MIN_PHASE_SECONDS, 120f)
    return mapIndexed { i, bp ->
        if (i in estIdx) bp.copy(time = perPhaseSec, condition = "weight", value = gv)
        else bp
    }
}
...
fun List<PhaseV3>.toBrewPhases(global: GlobalStopConditions? = null): List<BrewPhase> {
    ...
    return base.estimateVolumeDrivenTimes(globalVol)
}
